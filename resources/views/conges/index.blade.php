            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $requests->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $requests->where('status', 'approved')->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $requests->where('status', 'pending')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Working Days</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($requests as $request)
                            <tr class="hover:bg-slate-50 transition duration-150">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-slate-900">{{ \Carbon\Carbon::parse($request->start_date)->format('M d, Y') }}</div>
                                    <div class="text-xs text-slate-500">to {{ \Carbon\Carbon::parse($request->end_date)->format('M d, Y') }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-sm text-slate-700 max-w-xs truncate">{{ $request->reason ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @elseif($request->status === 'rejected')
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-semibold bg-red-50 text-red-700 border border-red-100">
                                            <svg class="h-4 w-4 mr-1.5" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if($request->status === 'pending')
                                            <form action="{{ route('conges.destroy', $request) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center px-3 py-1.5 text-sm rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition font-medium" onclick="return confirm('Cancel this request?')">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-slate-600 font-medium">No leave requests yet</p>
                                    <p class="text-slate-500 text-sm mt-1">Request a leave to get started</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
